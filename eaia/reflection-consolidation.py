...
async def update_general(state: ReflectionState, config, store: BaseStore):
    try:
        ...
        logger.error(f"Error in update_general: {e}")
        logger.exception(e)
...
async def determine_what_to_update(state: MultiMemoryInput, config, store: BaseStore):
    try:
        ...
        logger.error(f"Error in determine_what_to_update: {e}")
        logger.exception(e)
        # Return an empty command to avoid breaking the flow
        return Command(goto=[])
...
async def call_reflection(state: ReflectionState):
    try:
        ...
        logger.error(f"Error in call_reflection: {e}")
        logger.exception(e)
...
async def optimize_from_feedback(state: MemoryReflectionState, config, store: BaseStore):
    ...
    try:
        ...
        logger.error(f"Error in optimize_from_feedback: {e}")
        logger.exception(e)
        return {"status": "Error", "message": str(e)}


multi_reflection_graph = StateGraph(MemoryReflectionState)
