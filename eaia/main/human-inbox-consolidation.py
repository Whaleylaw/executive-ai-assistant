...
from eaia.memory import process_email_for_memory, convert_to_langchain_messages, _safely_prepare_messages_for_conversion
...
async def send_message(state: State, config, store):
    ...
    try:
        ...
        msg = {
            "type": "tool",
            "name": tool_call_name,
            "content": f"An error occurred while processing your response. Please try again.",
            "tool_call_id": tool_call_id,
        }
        return {"messages": [msg]}
...
async def send_email_draft(state: State, config, store):
    ...
    try:
        ...
        logger.error(f"Error in send_email_draft: {e}")
        msg = {
            "type": "tool",
            "name": tool_call_name,
            "content": f"An error occurred while processing your response. Please try again.",
...
async def send_cal_invite(state: State, config, store):
    ...
    try:
        ...
        logger.error(f"Error in send_cal_invite: {e}")
        msg = {
            "type": "tool",
            "name": tool_call_name,
            "content": f"An error occurred while processing your response. Please try again.",
