...
from eaia.main.triage_with_memory import (
    triage_input,
)
from eaia.main.draft_response_with_memory import draft_response
from eaia.main.find_meeting_time import find_meeting_time
from eaia.main.rewrite import rewrite
from eaia.main.config import get_config
from langchain_core.messages import ToolMessage
from eaia.main.human_inbox_with_memory import (
    send_message,
    send_email_draft,
    notify,
    send_cal_invite,
...
def take_action(
    ...
    # Handle memory tool calls - they should be followed by another draft step
    elif tool_call["name"] in ["ManageMemory", "SearchMemory"]:
...
async def process_memory_node(state: State, config, store):
    ...
    if tool_call["name"] == "ManageMemory":
        message = "Memory stored successfully. You can now continue drafting your response."
    elif tool_call["name"] == "SearchMemory":
        message = "Memory search completed. You can now use this information to inform your response."
    else:
...
async def initialize_memory_store(config=None):
    """Initialize the memory store during startup."""
    return setup_memory_store(config)
...
# Add new memory processing node
